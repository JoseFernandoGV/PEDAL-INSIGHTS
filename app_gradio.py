...
class DataEngineering:
    ...
    def agregar_variables_adicionales(self):
        ...
        # Frecuencia de uso de cada bicicleta
        print(self.data.select(pl.col("bike_id"),pl.col("trip_id")))
        ...
        self.data = self.data.with_columns(pl.col("bike_id").fill_null(strategy='mean'))
...
def predict(file):
    ...
    predictions = processed_data.select(["trip_id"]).with_columns(
        pl.Series(name="passholder_type", values=y_pred_labels)
    )
